...
class AdXGameSimulator:
    ...
    def run_simulation(self, agents: list[NDaysNCampaignsAgent], num_simulations: int) -> None:
        ...
        # campaign history
        campaign_history: List[List[Set[Campaign]]] = []

        total_profits = {agent : 0.0 for agent in agents}
        for i in range(num_simulations):

            # campaign history per game
            game_campaign_history: List[Set[Campaign]] = []
            ...
            for day in range(1, self.num_days + 1):
                ...
                # day campaign history
                day_campaign_history: Set[Campaign] = set()
                ...
                for agent in self.agents:
                    if random.random() < min(1, agent.quality_score):
                        ...
                        # log random campaigns
                        if agent.name == "Data Agent": continue # skip self
                        day_campaign_history.add(random_campaign)

                # store campaign day history
                game_campaign_history.append(day_campaign_history)

            # store game
            campaign_history.append(game_campaign_history)
