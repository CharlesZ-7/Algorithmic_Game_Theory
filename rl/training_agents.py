...
if __name__ == "__main__":
    ...
    test_agents += [randcamp_agent, campaign_agent, better_agent, lessrand_agent]
    test_agents += [RandomNDaysNCampaignsAgent(name=f"Rng Agent {i + 1}") for i in range(2)]
    test_agents += [CriticalNDaysNCampaignsAgent(name=f"Crit Agent {i + 1}") for i in range(1)]

    # Don't change this. Adapt initialization to your environment
    simulator = AdXGameSimulator()
    simulator.run_simulation(agents=test_agents, num_simulations=2000)
    ...
    # i want a concrete victor...
    # little less randomness...

    # mess with the randomness a little more...
    # throw out the losing peoples...