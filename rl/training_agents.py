...
# from rl_agent import RLModelNDaysNCampaignsAgent
...
class WF1NDaysNCampaignsAgent(BetterNDaysNCampaignsAgent):

    def __init__(self, name = "WF1 Agent"):
        super().__init__()
        self.name = name
...
class RLNDaysNCampaignsAgent(BetterNDaysNCampaignsAgent):
...
if __name__ == "__main__":
    ...
    test_agents = [WFNDaysNCampaignsAgent()] + [Tier1NDaysNCampaignsAgent(name=f"Agent {i + 1}") for i in range(9)]
    ...
    # test_agents += [randcamp_agent, campaign_agent, better_agent, lessrand_agent, WF1NDaysNCampaignsAgent(), bid_agent, wf_agent]
    ...
    # logging agent history for visualizations
    log = {}
    for agent in test_agents:
